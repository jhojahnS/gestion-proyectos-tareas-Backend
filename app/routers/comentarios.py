...
async def create_comentario(comentario: ComentarioCreate, service: ComentarioService = Depends()):
...
async def read_comentario(id: int, service: ComentarioService = Depends()):
    return service.get_by_id(id)


@router.patch("/{id}", response_model=ComentarioResponse)
async def update_comentario(id: int, comentario_data: ComentarioUpdate, service: ComentarioService = Depends()):
    return service.update(id, comentario_data)


@router.delete("/{id}", response_model=dict)
async def delete_comentario(id: int, service: ComentarioService = Depends()):
